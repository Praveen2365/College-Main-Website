...
$(document).ready(function() {
    ...
    marquee.addEventListener('mouseover', () => {
      marquee.stop(); // Stop the marquee on mouseover
    });

    marquee.addEventListener('mouseout', () => {
      marquee.start(); // Restart the marquee on mouseout
    });
    ...
    // <script>
    const marquee2 = document.getElementById('myMarquee2');
...
function getWindowWidth(){
    document.getElementById('set-width').style.maxWidth = window.screen.width.toString() + "px";
}

function toggleNabBar(){
  const navbtn = document.getElementById("navbarSupportedContent");
  navbtn.classList.toggle('navbar-toggle-content-show');
  const btn = document.getElementById('change-icon').children[0];
